...
public class IncorrectRegistration_test extends testBase {
    ...
    public void incorrectAllEmptyFields() throws InterruptedException, UnsupportedEncodingException {
        ...
        assertThat( app.getRegistrationHelper().checkError("error_register_user_name"), equalTo(Constants.getEmptyErrorLanguage(language)));
        log.log( Level.INFO, Constants.getEmptyErrorLanguage(language));
        assertThat( app.getRegistrationHelper().checkError("error_register_organisation"), equalTo(Constants.getEmptyErrorLanguage(language)));
        log.log( Level.INFO, Constants.getEmptyErrorLanguage(language));
        assertThat( app.getRegistrationHelper().checkError("error_register_email"), equalTo(Constants.getEmptyErrorLanguage(language)));
        log.log( Level.INFO, Constants.getEmptyErrorLanguage(language));
    }
   ...
   @Test(dataProvider = "registrationLongData", dataProviderClass = DataGenerator.class )
    public void incorrectAllLongFields(String longName, String longOrg, String longEmail) throws InterruptedException {
        ...
        app.getRegistrationHelper().fillRegistrationForm(
                ...
                longOrg,
                longEmail);
        assertThat(app.getRegistrationHelper().getValue("registrationName"), equalTo(longName.substring(0, 255)));
        assertThat(app.getRegistrationHelper().getValue("registrationOrg"), equalTo(longOrg.substring(0, 255)));
        assertThat(app.getRegistrationHelper().getValue("registrationEmail"), equalTo(longEmail.substring(0, 255)));
    }
     ...
     {      log.log(Level.INFO, "Incorrect test: email");
         app.getWebDriverHelper().openUrl(app.getRegistrationHelper().getRegistrationURL());
            ...
            app.getRegistrationHelper().fillRegistrationForm(
                    ...
                    incorrectEmail);
         app.getRegistrationHelper().clickRegisterButtonPage1();
         assertThat(app.getRegistrationHelper().checkError("error_register_email"), equalTo(Constants.getIncorrectEmailErrorLanguage(language)));
